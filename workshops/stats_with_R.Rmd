...
[Return to SCWG Home](https://camh-scwg.github.io/compucool-may-2017/)
...
```
```{r echo = FALSE}
messy_demographic <- read_csv("data/messy_demographic.csv")  # put in the location of the downloaded data file 1
messy_cognitive <- read_csv("data/messy_cognitive.csv")  # put in the location of the downloaded data file 2
messy_genotype <- read_csv("data/messy_genotype.csv")
```
# Running Intro to R commands
...
The linear model function in R, like many stats functions in R, uses *formula notation*, where you describe you dependant and independant variables, separated by a tilda `~`.
...
## getting stuff out of our summary

### augment grabs residuals and predicated values
...
The following line:
We observe a significant relationship between age adn cognitive scale 2 (t = `r my_t_stat`, p = `r cogbyage_tidy$p.value[2]`)
